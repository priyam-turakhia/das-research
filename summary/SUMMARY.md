...
| `scripts/mine_eval.py` | de–dsb eval of a distilled encoder (student embeds dsb, LaBSE embeds German). `--parallel` → CSLS retrieval P@1; default → BUCC mining precision/recall/F1 + threshold-free P@1. |
...
Parallel data is prepped by `scripts/prepare_parallel.py` (pair-aware: either-side dedup, GlotLID deu/pol, terminal-punct, length 3–80 + ratio ≤3): Europarl de–pl 579,166 → 529,522 clean → `data/processed/de-pl/{train,dev,test}.{de,pl}` = 476,569 / 26,476 / 26,477, every sentence unique on both sides.

First run (morfessor): in-domain Europarl retrieval P@1 went 0 → **0.94** (pl→de), proving the method — once the student embedding is L2-normalized before the MSE (without it the loss collapses to the target centroid). dsb transfer is **zero-shot** (dsb never in distillation): `scripts/mine_eval.py` gives chance → **0.136** on a clean 1,352-pair parallel set, but only **0.005 F1 / 0.021 retrieval P@1** on the PaSeMiLL BUCC test (44k×67k pool) — the weak transfer signal doesn't survive a realistic mining pool. The lever for a genuinely useful dsb encoder is direct de–dsb distillation. Full numbers in `docs/EVALUATIONS.md §10`, metrics in `docs/METRICS.md §8–9`.
